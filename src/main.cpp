...
unsigned int nStakeTargetSpacing = 50;		// 50 second block time

int64 nChainStartTime = 1404454027;
...
const string strMessageMagic = "OHLCcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight > 100 && nHeight <= 500)
       nSubsidy = 150 * COIN;

    else if(nHeight > 500 && nHeight <= 1500)
           nSubsidy = 500 * COIN;

    else if(nHeight > 1500 && nHeight <= 3000)
           nSubsidy = 1000 * COIN;

    else if(nHeight > 3000 && nHeight <= 10000)
           nSubsidy = 10 * COIN;

    else if(nHeight > 10000 && nHeight <= 11000)
           nSubsidy = 150 * COIN;

    else if(nHeight > 11000 && nHeight <= 12000)
           nSubsidy = 500 * COIN;

    else if(nHeight > 12000 && nHeight <= 12200)
           nSubsidy = 1000 * COIN;

    else if(nHeight > 12200 && nHeight <= 100000)
           nSubsidy = 10 * COIN;

    else if(nHeight > 100000 && nHeight <= 100500)
           nSubsidy = 150 * COIN;

    else if(nHeight > 100500 && nHeight <= 101000)
           nSubsidy = 100000 * COIN;

    else if(nHeight > 101000 && nHeight <= 101250)
           nSubsidy = 1000 * COIN;

    else if(nHeight > 101250 && nHeight <= 150000)
           nSubsidy = 10 * COIN;

    else if(nHeight > 150000 && nHeight <= 150250)
           nSubsidy = 150 * COIN;

    else if(nHeight > 150250 && nHeight <= 150500)
           nSubsidy = 500 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_OHLC_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in OHLCcoin
    bool fStrictPayToScriptHash = true; // Always active in OHLCcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "OHLCcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "04/07 - ALJAZEERA - Deaths as overpass collapses in Brazil city";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xc6d454455ed54838977e99bd1cecb28983df408e40862195ffae48f743b427da"));
